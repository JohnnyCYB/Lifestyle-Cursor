...
const petStyles = {
  dragon: { title: 'dragon', base: ['#ffb55f', '#f04432', '#78151d'], accent: '#ffd166', glow: '#fb6b22', wing: true, horns: true, flame: true, tail: 'flame' },
  ...
  return (
    <svg className={`monster-svg concept-art concept-${id} stage-${stage}`} viewBox="0 0 520 380" role="img" aria-label={`${style.title} evolution stage ${stage + 1}`}>
      ...
      <g transform={`translate(260 196) scale(${scale}) translate(-260 ${-196 + yLift})`} filter={`url(#${p('shadow')})`}>
        {style.wing && stage > 0 && <Wings uid={uid} stage={stage} />}
        {style.crystals && stage > 0 && <Crystals uid={uid} stage={stage} />}
        <Tail uid={uid} type={style.tail} stage={stage} />
        {final && style.flex && <FlexArms uid={uid} />}
        ...
        {style.ears && <Ears uid={uid} type={style.ears} />}
        ...
        <Mouth mood={mood} />
        <Details id={id} />
        {style.flame && <Flames uid={uid} final={final} />}
      </g>
    </svg>
  )
}
...
function Wings({ uid, stage }) { const final = stage === 3; const left = final ? 'M190 164 C126 82 70 36 34 58 C32 124 78 198 174 220 Z' : 'M200 174 C146 106 98 74 66 92 C70 150 108 202 182 214 Z'; const right = final ? 'M330 164 C394 82 450 36 486 58 C488 124 442 198 346 220 Z' : 'M320 174 C374 106 422 74 454 92 C450 150 412 202 338 214 Z'; return <g><path d={left} fill={`url(#${uid}-body)`} opacity="0.82"/><path d={right} fill={`url(#${uid}-body)`} opacity="0.82"/><path className="concept-line" d={final ? 'M180 204 L58 76 M340 204 L462 76 M132 186 L82 128 M388 186 L438 128' : 'M188 202 L82 102 M332 202 L438 102'} /></g> }
function Tail({ uid, type, stage }) { const final = stage === 3; const d = type === 'curl' ? 'M344 214 C428 178 430 84 368 70 C326 64 314 118 350 136' : type === 'blade' ? 'M350 200 C424 188 474 148 470 84 C408 96 358 138 334 190' : type === 'feather' ? 'M224 260 C160 328 205 342 250 286 C260 352 314 350 304 286 C358 338 404 320 328 258' : final ? 'M154 220 C68 250 34 192 80 154 C118 122 158 146 150 188' : 'M178 224 C108 242 86 196 118 170 C148 146 178 166 170 198'; return <path className="concept-tail" d={d} fill={`url(#${uid}-body)`} /> }
function FlexArms({ uid }) { return <g><path className="concept-arm" d="M152 174 C88 178 54 236 86 286 C144 292 178 252 184 194 Z" fill={`url(#${uid}-body)`}/><path className="concept-arm" d="M368 174 C432 178 466 236 434 286 C376 292 342 252 336 194 Z" fill={`url(#${uid}-body)`}/></g> }
function Legs({ uid }) { return <g><path className="concept-leg" d="M194 250 L164 322 L220 322 L232 252 Z" fill={`url(#${uid}-body)`}/><path className="concept-leg" d="M292 250 L322 322 L266 322 L254 252 Z" fill={`url(#${uid}-body)`}/><path className="concept-claw" d="M164 318 l-16 18 l28 -8 M196 322 l-6 22 l20 -18 M322 318 l16 18 l-28 -8 M290 322 l6 22 l-20 -18"/></g> }
function Horns({ uid, stage }) { const final = stage === 3; return <path className="concept-horn" d={final ? 'M188 90 L132 18 L220 72 M316 90 L374 18 L284 72 M246 72 L258 6 L278 74' : 'M206 104 L176 48 L230 90 M298 104 L328 48 L274 90'} fill={`url(#${uid}-metal)`} /> }
function Ears({ uid, type }) { if (type === 'sharp') return <path className="concept-ear" d="M178 118 L150 42 L220 98 M310 98 L370 42 L340 118" fill={`url(#${uid}-body)`}/>; return <g><ellipse className="concept-ear" cx="174" cy="136" rx="31" ry="36" fill={`url(#${uid}-belly)`}/><ellipse className="concept-ear" cx="330" cy="136" rx="31" ry="36" fill={`url(#${uid}-belly)`}/></g> }
function Armor({ uid, stage }) { return <g><path className="concept-armor" d={stage === 3 ? 'M176 176 L334 176 L310 262 L198 262 Z' : 'M198 188 L314 188 L296 250 L216 250 Z'} fill={`url(#${uid}-metal)`}/><path className="concept-line" d={stage === 3 ? 'M198 200 H312 M188 228 H322' : 'M214 208 H298 M210 232 H302'} /></g> }
function Crystals({ uid }) { return <g filter={`url(#${uid}-glow)`}><path className="concept-crystal" d="M166 102 L194 18 L230 108 Z"/><path className="concept-crystal" d="M322 112 L370 28 L392 126 Z"/></g> }
function Flames({ uid, final }) { return <g filter={`url(#${uid}-glow)`}><path className="concept-flame" d={final ? 'M386 78 C430 38 462 74 444 112 C424 154 382 136 366 194 C330 140 346 104 386 78 Z' : 'M360 94 C388 70 408 94 394 120 C380 146 356 134 348 170 C326 132 338 108 360 94 Z'} fill={`url(#${uid}-aura)`}/></g> }
function Details({ id }) { if (id === 'wolf') return <path className="concept-line cyber" d="M188 210 H334 M212 188 L170 150 M318 188 L364 150 M236 238 L220 282 M286 238 L302 282" />; if (id === 'titan') return <path className="concept-line stone" d="M152 198 H366 M142 230 H378 M178 272 H334 M196 94 L172 142 M336 94 L364 142" />; return <path className="concept-line" d="M204 194 H308 M194 220 H318 M210 248 H296" /> }
function Mouth({ mood }) { if (mood === 'silly') return <path className="concept-mouth" d="M218 158 C246 190 286 190 314 158" />; return <path className="concept-mouth" d="M214 158 C244 174 288 174 318 158" /> }
function Emotes({ mood, color }) { if (mood === 'happy') return <g className="concept-emote"><path d="M92 78 C76 48 28 70 54 108 C68 130 96 148 96 148 S128 128 140 108 C166 70 108 48 92 78 Z" fill={color}/></g>; if (mood === 'silly') return <g className="concept-emote"><path d="M92 88 C132 94 158 118 174 154 M428 100 C392 104 364 124 344 160"/><circle cx="408" cy="74" r="13" fill={color}/></g>; if (mood === 'power' || mood === 'celebrate' || mood === 'victory') return <g className="concept-emote"><path d="M72 146 L128 110 L108 170 L158 154 L82 230 L102 170 Z" fill={color}/><path d="M448 132 L398 102 L414 162 L370 150 L438 218 L422 162 Z" fill={color}/></g>; if (mood === 'curious') return <text className="pet-question" x="408" y="90">?</text>; return null }
