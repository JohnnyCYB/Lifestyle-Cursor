...
import { Activity, BadgeCheck, CalendarDays, CheckCircle2, Circle, Dumbbell, Flame, Flag, Heart, Home, Leaf, MoreHorizontal, PawPrint, Plus, RefreshCcw, Repeat2, Save, Settings2, Sparkles, Target, Trash2, Trophy, User, Utensils, Wand2, X, Zap } from 'lucide-react'
...
function App() {
  ...
  function switchAccountMode(nextMode) {
    window.localStorage.setItem(accountModeKey, nextMode)
  ...
  function saveGymTemplate(template) {
    setProfile((current) => ({ ...current, gymTemplates: getGymTemplates(current).map((item) => (item.id === template.id ? { ...item, ...template } : item)), ledger: [makeLedgerEntry(`Updated workout day: ${template.name}`, 0, 'gym'), ...current.ledger].slice(0, 10) }))
  ...
  function logGymTemplate(template) {
    setProfile((current) => {
      ...
      const workoutLogDates = existingDates.includes(todayKey) ? existingDates : [...existingDates, todayKey].slice(-60)
      ...
      return { ...updated, workoutLogDates, gymSessionCount: (current.gymSessionCount ?? 0) + 1, gymStreak: computeGymStreak(workoutLogDates) }
    })
  ...
  function completeGoal(goalId) {
    ...
    setProfile((current) => {
      ...
      const withCompleted = { ...updated, lifeGoals: updated.lifeGoals.map((item) => (item.id === goalId ? { ...item, points, completed: true, completedAt: todayKey } : item)) }
      return current.settings.autoRefillGoals ? refillGoalsToMinimum(withCompleted, 3) : withCompleted
    })
    triggerPetReaction(goalToComplete?.size === 'big' ? 'victory' : 'celebrate')
  }
  ...
  function feedPetSnack(rewardId = null) {
    if (rewardId) {
      ...
      if (!reward) return
      setProfile((current) => ({ ...current, rewardInventory: current.rewardInventory.filter((id) => id !== rewardId), activeXpMultiplier: reward.multiplier, activeXpMultiplierLabel: reward.title, care: applyCare(current.care, { satiety: 10, bond: 6, spark: 8 }), ledger: [makeLedgerEntry(`Fed ${reward.title}. Next XP is ${reward.multiplier}x`, 0, 'nutrition'), ...current.ledger].slice(0, 10) }))
      setSnackFed(true)
  ...
  function resetProfile() {
    setProfile(accountMode === 'demo' ? buildDemoProfile() : buildRealProfile())
    setSnackReady(false); setSnackFed(false); setLastFoodEntry(null); setActiveView('dashboard'); setPetReaction('idle')
  }

  const sharedProps = { profile, companion, stageIndex, stageName, evolution, completedToday, unlockedAchievements, weeklyTotal, petReaction, petCanMove, workoutForm, setWorkoutForm, goalForm, setGoalForm, winFilter, setWinFilter, nutritionFocus, setNutritionFocus, snackReady, snackFed, lastFoodEntry, todayKey, onActivity: addActivity, onCompleteQuest: completeQuest, onSelectPet: selectPet, onToggleSetting: toggleSetting, onTogglePetRoam: togglePetRoam, onMovePet: movePetTo, onResetProfile: resetProfile, onRecommendedWorkout: logGymTemplate, onSaveGymTemplate: saveGymTemplate, onSaveCustomWorkout: saveCustomWorkout, onLogCustomWorkout: (workout) => addActivity({ label: workout.name, points: workout.points, area: 'gym', reaction: 'power', care: { energy: -6, satiety: -3, bond: 5, spark: 6 } }), onAddLifeGoal: addLifeGoal, onCompleteGoal: completeGoal, onDeleteGoal: deleteGoal, onRefillGoals: refillGoalsNow, setActiveView, onFoodLog: handleFoodLog, onFeedSnack: feedPetSnack }
  ...
  return <div className={['app-shell', 'mobile-polish', profile.settings.animations ? 'motion-ok' : 'motion-off', profile.settings.focusMode ? 'focus-mode' : '', profile.demoMode ? 'demo-mode' : 'real-mode'].filter(Boolean).join(' ')}>
    ...
    <aside className="app-rail" aria-label="Primary navigation">
      <button className="brand-lockup" type="button" onClick={() => setActiveView('dashboard')}><span className="brand-mark"><PawPrint size={20} aria-hidden="true" /></span><span><strong>Life RPG</strong><small>{profile.demoMode ? 'Demo Account' : 'My Account'}</small></span></button>
    ...
    <main className="app-main">
      <header className="topbar" data-no-pet-move="true"><div><span className="eyebrow">Welcome back, {profile.userName}</span><h1>{activeTitle(activeView)}</h1></div><div className="topbar-actions"><AccountSwitcher mode={accountMode} onSwitch={switchAccountMode} /><MetricPill icon={Flame} label={`${profile.streak} day streak`} /><MetricPill icon={Sparkles} label={`${profile.points} pts`} /><button className="icon-toggle" type="button" aria-pressed={profile.settings.animations} title="Toggle animations" onClick={() => toggleSetting('animations')}><Settings2 size={18} aria-hidden="true" /></button></div></header>
...
function usePersistentProfile(accountMode) {
  const [profile, setProfileState] = useState(() => loadProfile(accountMode))
  useEffect(() => setProfileState(loadProfile(accountMode)), [accountMode])
  const setProfile = useCallback((updater) => { setProfileState((current) => { const next = normalizeProfile(mergeProfile(typeof updater === 'function' ? updater(current) : updater), accountMode); window.localStorage.setItem(storageKeyFor(accountMode), JSON.stringify(next)); return next }) }, [accountMode])
...
function buildRealProfile() { return normalizeProfile({ ...defaultProfile, userName: 'Player', points: 0, streak: 1, petType: 'dragon', lifeGoals: defaultProfile.lifeGoals, rewardInventory: [], unlockedRewardIds: [], petXp: { dragon: 0 }, demoMode: false }, 'real') }
function buildDemoProfile() { const today = localDateKey(); const rewardIds = unlockRewards.map((reward) => reward.id); const petXp = Object.fromEntries(companions.map((pet) => [pet.id, 6500])); return normalizeProfile({ ...defaultProfile, userName: 'Demo Player', demoMode: true, petType: 'monkey', points: 6500, streak: 12, care: { satiety: 92, energy: 88, bond: 96, spark: 90 }, skillXp: { gym: 1800, nutrition: 1400, focus: 900, life: 1200 }, weeklyPoints: [180, 260, 220, 310, 0, 0, 0], completedByDate: { [today]: ['hydrate', 'lift'] }, customWorkouts: [{ id: 'demo-push', name: 'Push Day', detail: 'Bench, incline press, shoulders, triceps', points: 70, createdAt: 'Demo' }], lifeGoals: [{ id: 'demo-win-1', title: 'Finish a full week of training', category: 'gym', size: 'big', points: 500, completed: true, completedAt: today }, { id: 'demo-goal-1', title: 'Hit protein goal today', category: 'nutrition', size: 'small', points: 50, completed: false, createdAt: 'Demo' }], rewardInventory: rewardIds, unlockedRewardIds: rewardIds, petXp, workoutLogDates: makeRecentDates(5), gymSessionCount: 18, gymStreak: 5, ledger: [makeLedgerEntry('Demo account loaded', 0, 'life')] }, 'demo') }
function normalizeProfile(profile, accountMode = profile?.demoMode ? 'demo' : 'real') { const petXp = { ...(profile?.petXp ?? {}) }; const activePet = profile?.petType ?? defaultProfile.petType; if (!Number.isFinite(petXp[activePet])) petXp[activePet] = Number.isFinite(profile?.points) ? profile.points : 0; return { ...profile, demoMode: accountMode === 'demo', gymTemplates: Array.isArray(profile.gymTemplates) ? profile.gymTemplates : getDefaultGymTemplates(), workoutLogDates: Array.isArray(profile.workoutLogDates) ? profile.workoutLogDates : [], gymSessionCount: Number.isFinite(profile.gymSessionCount) ? profile.gymSessionCount : 0, gymStreak: Number.isFinite(profile.gymStreak) ? profile.gymStreak : 0, unlockedRewardIds: Array.isArray(profile.unlockedRewardIds) ? profile.unlockedRewardIds : [], rewardInventory: Array.isArray(profile.rewardInventory) ? profile.rewardInventory : [], activeXpMultiplier: profile.activeXpMultiplier ?? null, activeXpMultiplierLabel: profile.activeXpMultiplierLabel ?? null, petXp } }
